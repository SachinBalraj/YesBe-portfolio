...
function Dot({ className = "" }: { className?: string }) {
  return (
    <span className={`mx-3 inline-block h-1 w-1 rounded-full bg-primary/25 ${className}`} />
  );
}
...
export function MarqueeSection() {
  ...
  return (
    <section className="relative border-y border-white/20 bg-gradient-to-r from-white via-[#f8fafc] to-white py-6 overflow-hidden">
      ...
      <div className="relative flex items-center w-max animate-marquee">
        {items.map((tech, i) => (
          <div key={`${tech}-${i}`} className="flex items-center shrink-0">
            <span className="whitespace-nowrap text-[15px] font-semibold tracking-wide text-foreground/60 transition-colors duration-300 hover:text-primary">
              {tech}
            </span>
            <Dot />
          </div>
