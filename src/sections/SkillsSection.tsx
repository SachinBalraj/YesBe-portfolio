...
function ProgressRing({ percent, size = 72, stroke = 5 }: { percent: number; size?: number; stroke?: number }) {
  ...
  return (
    <svg width={size} height={size} className="shrink-0 -rotate-90">
      <circle cx={size / 2} cy={size / 2} r={r} fill="none" stroke="#E5F0FF" strokeWidth={stroke} />
      <motion.circle
        cx={size / 2}
        cy={size / 2}
        r={r}
      ...
      <text
        ...
        className="fill-[#0F172A] text-[13px] font-bold"
...
const cardBase =
  "relative flex flex-col rounded-[20px] border border-[#E5F0FF] bg-white/80 backdrop-blur-sm p-6 transition-all duration-350 ease-out cursor-default";
...
function StatCard({ stat, index }: { stat: (typeof stats)[number]; index: number }) {
  ...
  return (
    <motion.div
      ...
      className="glass-card relative flex flex-col items-center rounded-[20px] border border-[#E5F0FF] bg-white/70 p-6 text-center backdrop-blur-sm"
    ...
    >
      ...
      <div className="mt-1 text-sm font-semibold text-[#0F172A]">{stat.label}</div>
      <div className="text-[12px] text-[#64748B]">{stat.sub}</div>
    </motion.div>
  );
}
    ...
    >
      ...
      <div className="mb-5 flex items-start justify-between">
        <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br from-[#dbeafe] to-[#eff6ff] transition-transform duration-350 group-hover:rotate-6">
          <Icon className="h-6 w-6 text-[#2563eb]" strokeWidth={1.8} />
        </div>
        <ProgressRing percent={skill.percent} />
      </div>

      {/* Title + Description */}
      <h3 className="text-lg font-bold text-[#0F172A]">{skill.title}</h3>
      <p className="mt-2 flex-1 text-[13px] leading-relaxed text-[#64748B]">{skill.description}</p>
      ...
      <div className="mt-4 flex flex-wrap gap-1.5">
        {skill.chips.map((c) => (
          <span
            key={c}
            className="rounded-full border border-[#E5F0FF] bg-white px-3 py-1 text-[11px] font-semibold text-[#2563eb] transition-all duration-200 hover:bg-[#2563eb] hover:text-white hover:-translate-y-0.5"
          >
            {c}
          </span>
        ))}
      ...
      <a
        href="#contact"
        className="mt-5 inline-flex items-center gap-1.5 text-[13px] font-semibold text-[#2563eb] transition-all duration-200 group/link hover:gap-2.5"
      >
        Explore Skills
      ...
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <p className="mt-5 text-[15px] leading-relaxed text-[#64748B]">
            I build scalable digital solutions using modern technologies, AI, cloud platforms, and business automation.
          </p>
        </motion.div>
        ...
        >
          <h3 className="text-xl font-bold text-[#0F172A]">Technologies I Work With</h3>
        </motion.div>
        ...
        >
          {allChips.map((c, i) => (
            <motion.span
              ...
              className="rounded-full border border-[#E5F0FF] bg-white px-4 py-2 text-[13px] font-semibold text-[#0F172A] shadow-sm transition-colors duration-200 hover:border-[#2563eb] hover:bg-[#2563eb] hover:text-white cursor-default"
            >
              {c}
            </motion.span>
          ))}
        ...
        <div className="relative">
          ...
          <div className="overflow-hidden rounded-2xl border border-[#E5F0FF] bg-[#f8fbff] py-5">
            <div className="flex w-max animate-marquee hover:[animation-play-state:paused]">
              {[...marqueeItems, ...marqueeItems, ...marqueeItems].map((t, i) => (
                <span key={`${t}-${i}`} className="mx-6 whitespace-nowrap text-[14px] font-semibold text-[#0F172A]/60 transition-colors duration-200 hover:text-[#2563eb]">
                  {t}
